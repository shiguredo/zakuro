...
- `CMakeLists.txt` は `OPENSSL_IS_BORINGSSL` を定義しており、`buildbase.py` が BoringSSL を OpenSSL 互換としてインストールするため、実際に使われるのは OpenSSL ではなく BoringSSL である (issues/0009 参照)
...
- BoringSSL のライセンス (BSD 系 + 特許条項) を確認し、`NOTICE` に既存セクションと同じ形式 (ライブラリ名 + ソース URL + ライセンス本文) で追記する
